...
# cointaft_hash
RUN git clone https://github.com/TaftProject/cointaft_hash
RUN cd cointaft_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} cointaft
RUN useradd -u ${USER_ID} -g cointaft -s /bin/bash -m -d /cointaft cointaft
...
RUN mkdir /cointaft-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /cointaft-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /cointaft-src
