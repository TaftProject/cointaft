...
const char * const BITCOIN_CONF_FILENAME = "cointaft.conf";
const char * const BITCOIN_PID_FILENAME = "cointaftd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "cointaftd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CointaftCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CointaftCore
    // Mac: ~/Library/Application Support/CointaftCore
    // Unix: ~/.cointaftcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CointaftCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CointaftCore";
#else
    // Unix
    return pathRet / ".cointaftcore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty cointaft.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Cointaft Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Cointaft Core developers";
    }
