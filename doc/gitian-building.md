...
*Setup instructions for a Gitian build of Cointaft Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Cointaft
...
- [Building Cointaft Core](#building-cointaft-core)
...
Clone the git repositories for Cointaft Core and Gitian.
...
in the Cointaft Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Cointaft Core
----------------

To build Cointaft Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Cointaft Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/cointaft/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/TaftProject/cointaft
...
For example:
```bash
URL=https://github.com/crowning-/cointaft.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit cointaft=${COMMIT} --url cointaft=${URL} ../cointaft/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit cointaft=${COMMIT} --url cointaft=${URL} ../cointaft/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit cointaft=${COMMIT} --url cointaft=${URL} ../cointaft/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the cointaft git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../cointaft/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/cointaft
SIGPATH=/some/root/path/cointaft-detached-sigs

./bin/gbuild --url cointaft=${BTCPATH},signature=${SIGPATH} ../cointaft/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/cointaft-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/cointaft-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/cointaft-osx-build.assert
```
...
[cointaft/gitian.sigs](https://github.com/TaftProject/gitian.sigs/) repository, or if that's not possible to create a pull
request.
