...
PACKAGE_VERSION=$($SRC_PATH/src/cointaftd --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/cointaftd $DEB_BIN
cp $SRC_PATH/src/cointaft-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/cointaft-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/cointaftd.1 $DEB_MAN
cp $SRC_DOC/man/cointaft-cli.1 $DEB_MAN
cp $SRC_DOC/man/cointaft-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/cointaftd.bash-completion $DEB_CMP/cointaftd
cp $SRC_PATH/contrib/cointaft-cli.bash-completion $DEB_CMP/cointaft-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/cointaftd.1
gzip --best -n $DEB_MAN/cointaft-cli.1
gzip --best -n $DEB_MAN/cointaft-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/cointaftd $DEB_BIN/cointaft-cli
