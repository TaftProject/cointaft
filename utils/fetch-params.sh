...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/CointaftParams"
else
    PARAMS_DIR="$HOME/.cointaft-params"
fi
...
DOWNLOAD_URL="https://github.com/TaftProject/cointaft/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with TAFT_DISABLE_SOMETHING=1
TAFT_DISABLE_WGET="${TAFT_DISABLE_WGET:-}"
#TAFT_DISABLE_IPFS="${TAFT_DISABLE_IPFS:-}"
TAFT_DISABLE_CURL="${TAFT_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$TAFT_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$TAFT_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$TAFT_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Cointaft parameters!
...
main() {
    ...
    cat <<EOF
TAFT - fetch-params.sh

This script will fetch the Cointaft SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Cointaft SNARK parameters. Note that it is
